...
export class EddyService {

  private data: Eddy = new Eddy();
  public stepCount = 4;
  ...
  public async postInspectionData(): Promise<boolean> {
    ...
    return await this._dataProcessor.dataProcessor(this.data, cqmId, riserModel, 'Eddy Current')
      .then((res) => res)
      .catch((error) => error);
  }
  ...
  public returnTemplateDataObject(): Eddy {
    return this.data;
  }

  public setData(data: Eddy): void {
    this.data = data;
  }
  ...
  public async searchPreviousData(type: string): Promise<Eddy> {
  ...
  public screenNavigation(routeDirections: string, route: number) {
    ...
    switch (routeDirections) {
      case 'next':
        this.navCtrl.navigateForward(`inspections/eddy/${route + 1}`);
        break;
      case 'previous':
        this.navCtrl.navigateBack(`inspections/eddy/${route - 1}`);
        break;
      case 'exit':
