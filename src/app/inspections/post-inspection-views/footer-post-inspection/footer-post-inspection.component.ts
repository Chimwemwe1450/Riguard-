...
@Component({
  selector: 'app-footer-post-inspection',
  templateUrl: './footer-post-inspection.component.html',
  styleUrls: ['./footer-post-inspection.component.scss'],
})
export class FooterPostInspectionComponent implements OnInit {
  ...
  public async onSubmit(): Promise<void> {
    ...
    if (this.isFormValid) {
      this._postInspectionService.setFormAsComplete();
      this._postInspectionService.storePreviousData();
      ...
      await this._loadingService.presentLoading('Submitting Inspection...')
        .then(async () => {
          ...
          if (this.connection.status.connected) {
            // submit inspection.
            await this._postInspectionService.postInspectionData()
            .then((res: boolean) => {
              ...
              if (res) {
                ...
                this._postInspectionService.screenNavigation('exit', this.currentRoute);
  ...
  public onNext(): void {
    ...
    if (this.isFormValid) {
      this._postInspectionService.screenNavigation('next', this.currentRoute);
    } else {
  ...
  public onPrevious(): void {
    this._postInspectionService.screenNavigation('previous', this.currentRoute);
  ...
  private storeInspection(message: string): void {
    this._postInspectionService.storeInspectionData(this.currentRoute)
      .then(res => {
