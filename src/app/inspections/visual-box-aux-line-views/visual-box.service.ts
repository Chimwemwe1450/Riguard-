...
import { VisualBoxAux } from './visual-box.model';
import { EntityPostingDataProcessorService } from '../shared/services/entity-posting-data-processor.service';

@Injectable({
  providedIn: 'root'
})
export class VisualBoxAuxService {

  private data: VisualBoxAux = new VisualBoxAux();
  public stepCount = 3;
  ...
  public returnTemplateDataObject(): VisualBoxAux {
    return this.data;
  }

  public setData(data: VisualBoxAux): void {
    this.data = data;
  }
  ...
  public screenNavigation(routeDirections: string, route: number) {
    ...
    switch (routeDirections) {
      case 'next':
        this.navCtrl.navigateForward(`inspections/visual-box-aux/${route + 1}`);
        break;
      case 'previous':
        this.navCtrl.navigateBack(`inspections/visual-box-aux/${route - 1}`);
        break;
      case 'exit':
  ...
  private subscribeToRetrievedInspection() {
    this._dataStorage.$retrievedInspection.subscribe(data => {
      this.data = data || new VisualBoxAux();
    });
  }
