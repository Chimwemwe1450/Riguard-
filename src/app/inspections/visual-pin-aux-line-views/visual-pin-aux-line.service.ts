...
import { VisualPinAux } from './visual-pin-aux-line.model';
import { EntityPostingDataProcessorService } from '../shared/services/entity-posting-data-processor.service';

@Injectable({
  providedIn: 'root'
})
export class VisualPinAuxService {

  private data: VisualPinAux = new VisualPinAux();
  public stepCount = 3;
  ...
  public returnTemplateDataObject(): VisualPinAux {
    return this.data;
  }

  public setData(data: VisualPinAux): void {
    this.data = data;
  }
  ...
  public screenNavigation(routeDirections: string, route: number) {
    ...
    switch (routeDirections) {
      case 'next':
        this.navCtrl.navigateForward(`inspections/visual-pin-aux/${route + 1}`);
        break;
      case 'previous':
        this.navCtrl.navigateBack(`inspections/visual-pin-aux/${route - 1}`);
        break;
      case 'exit':
  ...
  private subscribeToRetrievedInspection() {
    this._dataStorage.$retrievedInspection.subscribe(data => {
      this.data = data || new VisualPinAux();
    });
  }
