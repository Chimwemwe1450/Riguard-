...
@Component({
  selector: 'app-footer-visual-pin',
  templateUrl: './footer-visual-pin-aux-line.component.html',
  styleUrls: ['./footer-visual-pin-aux-line.component.scss'],
})
export class FooterVisualPinAuxComponent implements OnInit {
  ...
  public async onSubmit(): Promise<void> {
    ...
    if (this.isFormValid) {
      this._visualPinService.setFormAsComplete();
      ...
      await this._loadingService.presentLoading('Submitting Inspection...')
        .then(async () => {
          ...
          if (this.connection.status.connected) {
            // submit inspection.
            await this._visualPinService.postInspectionData()
            .then((res: boolean) => {
              ...
              if (res) {
                ...
                this._visualPinService.screenNavigation('exit', this.currentRoute);

              } else {
                this.runIncrement();
                this.storeInspection('Submission failed, inspection stored.');

                this._toastService.presentToast('', 'primary');
                this._inspectionsListService.removeQcp();
                this._filteringService.filter(this._inspectionsListService.qcps);
              }
  ...
  public onNext(): void {
    ...
    if (this.isFormValid) {
      this._visualPinService.screenNavigation('next', this.currentRoute);
    } else {
  ...
  public onPrevious(): void {
    this._visualPinService.screenNavigation('previous', this.currentRoute);
  ...
  private storeInspection(message: string): void {
    this._visualPinService.storeInspectionData(this.currentRoute)
      .then(res => {
