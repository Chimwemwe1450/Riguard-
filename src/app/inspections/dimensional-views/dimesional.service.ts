...
export class DimensionalService {
  ...
  private data: Dimensional = new Dimensional();
  public stepCount = 1;
  ...
  public async postInspectionData(): Promise<boolean> {
    ...
    return await this._dataProcessor.dataProcessor(this.data, cqmId, riserModel, 'Dimensional')
      .then((res) => res)
      .catch((error) => error);
  }
  ...
  public returnTemplateDataObject(): Dimensional {
    return this.data;
  }

  public setData(data: Dimensional): void {
    this.data = data;
  }
  ...
  public async searchPreviousData(type: string): Promise<Dimensional> {
  ...
  public screenNavigation(routeDirections: string, route: number) {
    ...
    switch (routeDirections) {
      case 'next':
        this.navCtrl.navigateForward(`inspections/dimensional/${route + 1}`);
        break;
      case 'previous':
        this.navCtrl.navigateBack(`inspections/dimensional/${route - 1}`);
        break;
      case 'exit':
  ...
  private subscribeToRetrievedInspection() {
    this._dataStorage.$retrievedInspection.subscribe(data => {
      this.data = data || new Dimensional();
    });
  }
}
