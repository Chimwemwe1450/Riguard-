...
@Component({
  selector: 'app-footer-dye-pen',
  templateUrl: './footer-dye-pen.component.html',
  styleUrls: ['./footer-dye-pen.component.scss'],
})
export class FooterDyePenComponent implements OnInit {
  ...
  public async onSubmit(): Promise<void> {
    ...
    if (this.isFormValid) {
      this._dyePenService.setFormAsComplete();
      this._dyePenService.storePreviousData();
      ...
      await this._loadingService.presentLoading('Submitting Inspection...')
        .then(async () => {
          ...
          if (this.connection.status.connected) {
            // submit inspection.
            await this._dyePenService.postInspectionData()
            .then((res: boolean) => {
              ...
              if (res) {
                ...
                this._dyePenService.screenNavigation('exit', this.currentRoute);
  ...
  public onNext(): void {
    ...
    if (this.isFormValid) {
      this._dyePenService.screenNavigation('next', this.currentRoute);
    } else {
  ...
  public onPrevious(): void {
    this._dyePenService.screenNavigation('previous', this.currentRoute);
  ...
  private storeInspection(message: string): void {
    this._dyePenService.storeInspectionData(this.currentRoute)
      .then(res => {
