...
import { VisualBox } from './visual-box.model';
import { EntityPostingDataProcessorService } from '../shared/services/entity-posting-data-processor.service';

@Injectable({
  providedIn: 'root'
})
export class VisualBoxService {

  private data: VisualBox = new VisualBox();
  public stepCount = 5;
  ...
  public returnTemplateDataObject(): VisualBox {
    return this.data;
  }

  public setData(data: VisualBox): void {
    this.data = data;
  }
  ...
  public screenNavigation(routeDirections: string, route: number) {
    ...
    switch (routeDirections) {
      case 'next':
        this.navCtrl.navigateForward(`inspections/visual-box-main/${route + 1}`);
        break;
      case 'previous':
        this.navCtrl.navigateBack(`inspections/visual-box-main/${route - 1}`);
        break;
      case 'exit':
