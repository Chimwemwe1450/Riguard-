...
@Component({
  selector: 'app-ut-wall-views-step-five',
  templateUrl: './ut-wall-views-step-five.page.html',
  styleUrls: ['./ut-wall-views-step-five.page.scss'],
})
export class UtWallViewsStepFivePage implements OnInit {
  ...
  public stepDescription = 'D Reading';
  public validForm = true;

  data: ReadingLabels[] = [];
  dataSource = new BehaviorSubject<AbstractControl[]>([]);
  public rows: FormArray = this.fb.array([]);
  public form: FormGroup = this.fb.group({ readingD: this.rows });
  ...
  private createForm() {
    ...
    this.form = this.fb.group({ readingD: this.rows });
    // this.addRow();
  }
  ...
  public usePreviousData(): void {
    ...
    formData.readingC.forEach((data) => {
  ...
  private patchForm() {
    ...
    if (formData.readingD) {

      formData.readingD.forEach((data) => {
        const row = this.buildFormGroup(data);
        this.rows.push(row);
      });
    } else {
